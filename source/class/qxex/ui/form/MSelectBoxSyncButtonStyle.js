...
qx.Mixin.define("qxex.ui.form.MSelectBoxSyncButtonStyle", {
  ...
  members: {
    synchronizeButtonWithSelection(atom, listItems, total, labelFun, hasIcons, iconFun) {
      ...
      if (length == 0) {
        ...
        if (hasIcons) {
          ...
          if (!allHaveSameIcon) {
            if (this.getSomeIcon()) icon = this.getSomeIcon();
            //icon that displays that items have different icons
            else {
